                                    ...
                                    <div class="modal-dialog modal-xl modal-dialog-centered">
                                        <div class="modal-content rounded-4 shadow">
                                            ...
                                            <div class="modal-body text-center">
                                                @php
                                                    $fileUrl = asset('storage/' . $cert->authority_certificate_path);
                                                    $extension = pathinfo($cert->authority_certificate_path, PATHINFO_EXTENSION);
                                                @endphp

                                                @if(in_array(strtolower($extension), ['jpg','jpeg','png','gif']))
                                                    <img src="{{ $fileUrl }}" alt="Certificate Image" class="img-fluid rounded shadow">
                                                @elseif(strtolower($extension) === 'pdf')
                                                    <iframe src="{{ $fileUrl }}" width="100%" height="600px" frameborder="0"></iframe>
                                                @else
                                                    <a href="{{ $fileUrl }}" target="_blank" class="btn btn-outline-info">Download File</a>
                                                @endif
                                            </div>
                                        </div>
                                    </div>
